...
export async function GET(req: NextRequest) {
  ...
  const status = url.searchParams.get("status") || ""; // PUBLISHED/DRAFT/etc
  const sort = url.searchParams.get("sort") || "date";

  // Status mapping (UI -> WP)
  ...
  const c = await wpPrisma.$queryRawUnsafe<any[]>(`
    SELECT
      CAST(SUM(post_status IN ('publish','future','draft','private','trash')) AS UNSIGNED) as allCount,
